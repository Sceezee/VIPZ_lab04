...
void ForDelete::on_DeleteCondition_clicked()
{
    ...
    while (pHead)
    {
        ...
        YearCond = pHead->GetYear();
        PagesCond = pHead->GetPages();

        if (YearCond < 2000 && PagesCond < 150)
        {
            List* pTemp = pHead;
            pHead = pHead->GetNext();
            delete pTemp;
    ...
    while (pRun)
    {
        ...
        YearCond = pRun->GetYear();
        PagesCond = pRun->GetPages();

        if ((YearCond < 2000 && PagesCond < 150) && pRun->GetNext() == nullptr)
        {
